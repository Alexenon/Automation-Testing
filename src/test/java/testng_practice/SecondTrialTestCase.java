...
/**
 * This whole class belong to "first_group" but some of them belongs to "init"
 */
@Test(groups = {"first_group"})
public class SecondTrialTestCase {
    ...
    public void accessPage(@Optional("numberOfTimes") int numberOfTimes) {
        Assert.assertTrue(numberOfTimes > 0, "Number of times isn't valid");
    }

    @Test(
            dependsOnMethods = "accessPage",
            description = "Should be ignored because it's depending on accessPage"
    )
    public void dependsOnTest() {
        Assert.assertTrue(Boolean.TRUE, "Test 'passTest' is missing or failed!");
    }
